...
class Game:
    ...
    def reset_ball_and_bar(self):
        ...
        self.reward = 0
        self.vx = int(self.ball_speed * math.cos(self.ball_angle))
        self.vy = int(self.ball_speed * math.sin(self.ball_angle))
    ...
    def step(self, action): 
        ...
        self.skip_step = False
        if self.game_mode == "human":
            if keyboard.is_pressed("up") or keyboard.is_pressed("down") or keyboard.is_pressed("enter") or keyboard.is_pressed("shift") or keyboard.is_pressed("esc") or keyboard.is_pressed("right shift"):
                key = keyboard.read_key()
                while key != "down" and key != "up" and key != "enter" and key != "shift" and key != "right shift" and key != "esc":
                    key = keyboard.read_key()
                if key == "esc":
                    sys.exit()
        ...
        if self.pause == False and self.skip_step == False:   
            ...
            if self.pause == False:
                ...
                if self.bar_top + 1 - self.action > 0 and self.bar_top + 1 - self.action + self.bar_length - 1 < 159:
                    self.bar_top = self.bar_top + (1 - self.action) * bar_speed  # 0:down, 1:no move, 2:up
    ...
    def update_ball_position(self):
        new_x = int(self.ball_position[0] + self.vx)
        new_y = int(self.ball_position[1] + self.vy)
        if 0 <= new_x <= 209 and 6 <= self.ball_position[0] <= 203:
            while new_y <= 0 or new_y >= 159 or (new_x <= 6 and self.bar_top <= new_y <= self.bar_top + self.bar_length - 1) or (new_x >= 203 and self.opponent_bar_top <= new_y <= self.opponent_bar_top + self.bar_length - 1):
                if new_y <= 0 or new_y >= 159:
                    ...
                    self.vx = -self.vx
                    new_x = self.ball_position[0]
                    self.ball_speed = 3
                    
                new_x = int(new_x + self.vx)
                new_y = int(new_y + self.vy)
        ...
        if self.game_mode =="human":
            ...
            while True:
                if self.pause == False:
                    ...
                    pygame.display.update()
                    clock.tick(120)
                    if self.stop_one_sec or self.skip_step:
                        time.sleep(0.5)
                else:
                    key = keyboard.read_key()
                    if key == "enter":
